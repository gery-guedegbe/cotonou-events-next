...
export function AdminSidebar({ tab, onTabChange, pendingCount }: NavProps) {
  return (
    <aside className="sticky top-0 hidden h-screen w-60 flex-none flex-col border-r border-gray-200 bg-white p-5 md:flex">
      <div className="px-2.5 pb-5 pt-1.5 text-lg font-extrabold tracking-display">
      ...
      <nav className="flex flex-col gap-1">
        {NAV.map((n) => (
          <button
            ...
            className={cn(
              "flex items-center gap-3 rounded-[10px] px-3 py-2.5 text-left text-sm font-semibold transition-colors",
              tab === n.key
          ...
          >
            ...
            {n.key === "pending" && pendingCount > 0 && (
              <span className="flex h-5 min-w-[20px] items-center justify-center rounded-pill bg-red-500 px-1.5 text-2xs font-bold text-white">
                {pendingCount}
              </span>
            )}
          </button>
      ...
      <form action={signOutAdmin} className="mt-auto">
        <button
          type="submit"
          className="flex w-full items-center gap-3 rounded-[10px] px-3 py-2.5 text-left text-sm font-semibold text-gray-500 hover:bg-gray-100"
        >
        ...
        {NAV.map((n) => (
          <button
            ...
            className={cn(
              ...
              tab === n.key ? "text-brand" : "text-gray-500",
            )}
          >
            <n.Icon className="h-5 w-5" aria-hidden />

            <span className="text-2xs font-semibold">{n.short}</span>

            {n.key === "pending" && pendingCount > 0 && (
              <span className="absolute right-[18%] top-0.5 flex h-[15px] min-w-[15px] items-center justify-center rounded-pill bg-red-500 px-1 text-2xs font-bold text-white">
                {pendingCount}
              </span>
            )}
          </button>
